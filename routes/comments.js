...
router.get("/campgrounds/:id/comments/:comment_id/edit",middleware.checkCommentOwnership,(req,res)=>{
	...
	Comment.findById(req.params.comment_id,(err,foundcomm)=>{
		if(err){
			...
			res.render("comments/editcomment",{campground_id:req.params.id,comment:foundcomm});
		}
	});
});
...
router.put("/campgrounds/:id/comments/:comment_id",middleware.checkCommentOwnership,(req,res)=>{
	Comment.findByIdAndUpdate(req.params.comment_id,req.body.comment,(err,ucomm)=>{
		if(err){
			res.redirect("back");
		}else{
